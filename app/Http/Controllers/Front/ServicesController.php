...
class ServiceController extends Controller
{
    public function listing()
    {
        ...
        if ($slug) {
            // Cek apakah unit ada
            $unit = Unit::where(['slug' => $slug, 'is_active' => true])->first();

            if (! $unit) {
                abort(404);
            }

            $unitDetails = Unit::unitDetails($slug);
            $unitService = Service::with('unit', 'slides')
                ->whereIn('unit_id', $unitDetails['unitIds'])
                ->where('status', 1)
                ->orderBy('id', 'desc')
                ->simplePaginate();

            return view('front.services.listing', compact('unitDetails', 'unitService', 'page_title'));
        }
